...
class YamlToXmlConverter:
    ...
    def valuesGlue(self):
        ...
        for line in primitive_list:
            if type(line) != int and len(re.findall(':', str(line))) == 0:
                if '-' not in line or (buffer != '' and type(buffer[-1]) != int):  # here we check the position of '-'
                    buffer = buffer + str(line) + space
                if len(re.findall('\n', str(line))) > 0 or re.match(str(line), str(primitive_list[-1])):
                    if re.match(str(line), str(primitive_list[-1])):
                        buffer = buffer[:-1] + '\n'
    ...
    def correctSpaceYamlList(self):
        ...
        for ind in range(len(yaml_list)):
            item = yaml_list[ind]
            if type(item) != int and str(item).count(':') != 1 and len(re.findall('\n', str(item))) > 0:
    ...
    def nearest_tag_for_closing_finder(self, some_list, cycle_iterator, current_yaml_list):
        ...
        while True:
            ...
            if type(tmpItem) == int:
                ...
                if len(re.findall(str(yaml_list[iterator - 1]), str(some_list))) > 0:
                    break
            iterator -= 1
        tag = '/' + str(tmpItem)
        return tag, tmpItem
    ...
    def mainPart(self):
        xml = open('xmlRegFile.xml', 'w', encoding='utf-8')
